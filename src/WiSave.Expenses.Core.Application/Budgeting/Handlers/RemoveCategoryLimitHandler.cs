...
public sealed class RemoveCategoryLimitHandler(IAggregateRepository<Budget> repository)
{
    public async Task<CommandResult> HandleAsync(RemoveCategoryLimit command, CancellationToken ct = default)
    {
        try
        {
            ...
            budget.RemoveCategoryLimit(command.CategoryId);
